...
class Wiki(commands.Cog):
    ...
    async def wiki(self, intr: discord.Interaction, query: str):
        ...
        try:
            await intr.response.send_message(
                embed=self.get_embed_from_wiki_page(details), view=WikiView(details.url)
            )
    ...
    def get_embed_from_wiki_page(self, page_details):
        embed = discord.Embed(
            title=page_details.title, url=page_details.url, color=discord.Color.teal()
        )
        if page_details.description:
            embed.add_field(
                name="Description", value=page_details.description, inline=True
            )
        embed.add_field(
            name="Summary",
...
class WikiView(discord.ui.View):
    ...
    def __init__(self, url: str):
        ...
        self.add_item(
            discord.ui.Button(
                label="Wikipedia", url=self.url, style=discord.ButtonStyle.url
            )
        )
