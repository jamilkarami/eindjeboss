...
class Reminder(commands.Cog):
    ...
    async def remindme(
        self, intr: discord.Interaction, r_time: str, msg: str, daily: bool
    ):
        ...
        if p_ts < time.time():
            await intr.followup.send(
                "Stop living in the past, child. Look to the future.", ephemeral=True
            )
            return
        ...
        reminder = mk_reminder(
            rem_id, rem_time, msg, intr.guild_id, daily, intr.user.id, rem_msg.id
        )

        await self.save_reminder(reminder)

    @app_commands.command(
        name="myreminders", description="Get a list of your active reminders."
    )
    ...
    @app_commands.command(
        name="deletereminder", description="Don't get notified for a reminder."
    )
    ...
    async def deletereminder(self, intr: discord.Interaction, rem_id: str):
        ...
        if removed:
            ...
            return
        await intr.response.send_message(
            "This reminder doesn't exist or you weren't subscribed to it.",
            ephemeral=True,
        )

    @deletereminder.autocomplete("rem_id")
    async def delete_reminder_autocomplete(
        self, intr: discord.Interaction, current: str
    ):
        user_reminders = await self.get_user_reminders(intr.user.id)
